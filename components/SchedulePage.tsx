...
const BAR_COLORS = ["#2563eb", "#7c3aed", "#d97706", "#059669", "#dc2626"];
...
export function SchedulePage({ schedule, order, onApprovalDecision }: Props) {
    ...
    return (
      <div className="bg-gray-100 rounded-xl p-6">
        <div className="bg-white rounded-xl border border-gray-200 p-10 text-center shadow-sm">
          ...
          <p className="text-sm text-gray-500">No schedule yet. Submit an order from the Orders page to see the AI schedule here.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5 bg-gray-100 p-6 rounded-xl">
      {/* Header */}
      <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
        <div className="flex items-start justify-between mb-4">
          <div>
            <h3 className="text-sm font-semibold text-gray-900">Schedule for {order.id}</h3>
          ...
          <div className="mb-4 rounded-lg border border-amber-200 bg-amber-50 p-3">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="mt-0.5 text-xs text-amber-700">Review the schedule before releasing this order to production.</p>
              </div>
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="bg-gray-50 border border-gray-200 rounded-lg p-3">
            ...
            <p className="text-base font-semibold text-gray-900">{format(new Date(schedule.overallFinish), "h:mm a")}</p>
          </div>
          <div className="bg-gray-50 border border-gray-200 rounded-lg p-3">
            <div className="flex items-center gap-1.5 mb-1">
              {schedule.slaStatus === "SAFE" ? <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" /> : <AlertTriangle className="w-3.5 h-3.5 text-red-600" />}
              <p className="text-xs text-gray-500">Deadline</p>
            </div>
            <p className="text-base font-semibold text-gray-900">{format(new Date(order.deadline), "h:mm a")}</p>
          </div>
          <div className="bg-gray-50 border border-gray-200 rounded-lg p-3">
            <div className="flex items-center gap-1.5 mb-1">
            ...
            <p className="text-base font-semibold text-gray-900">{schedule.tasks.length}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {/* AI steps */}
        <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
          <h3 className="text-sm font-semibold text-gray-900 mb-4">AI decision steps</h3>
          <div className="space-y-3">
            {STEP_LABELS.map((step, i) => (
              <div key={i} className="flex items-start gap-3">
                <div className="w-6 h-6 rounded-full bg-blue-100 text-blue-700 flex items-center justify-center flex-shrink-0 text-xs font-semibold">{i + 1}</div>
                <div>
          ...
          <div className="space-y-2 mt-4">
            {schedule.tasks.map((t, i) => (
              <div key={t.machineId} className="flex items-center gap-3">
                ...
                <span className="text-xs text-gray-500">{t.assignedQty.toLocaleString()} · {t.estimatedHours}h</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
        <div className="flex items-center justify-between border-b border-gray-200 px-5 py-3">
          <div>
            <h3 className="text-sm font-semibold text-gray-900">Execution tracking</h3>
        ...
        <div className="divide-y divide-gray-100">
          {executionRows.map((task, i) => (
            <div key={task.machineId} className="grid gap-3 px-5 py-4 md:grid-cols-[110px_1fr_130px_150px] md:items-center">
              <div>
                <p className="text-sm font-semibold text-gray-900">{task.machineId}</p>
              ...
              <div>
                ...
                <p className={`text-sm font-semibold ${Math.abs(task.variance) > 10 ? "text-amber-700" : "text-gray-900"}`}>
      ...
      {schedule.risk && (
        <div className={`rounded-xl border p-5 ${
          ...
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <p className={`text-sm font-semibold ${
                schedule.risk.riskLevel === "HIGH" ? "text-red-700"
                : schedule.risk.riskLevel === "MEDIUM" ? "text-amber-700"
                : "text-emerald-700"
              }`}>AI Risk Analysis</p>
            </div>
            <div className="flex items-center gap-2">
          ...
          <p className="text-sm text-gray-800">{schedule.risk.recommendation}</p>
        </div>
      )}
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
        <div className="flex items-center gap-2 mb-3">
          ...
          <p className="text-sm font-semibold text-gray-900">Gemini AI explanation</p>
        </div>
